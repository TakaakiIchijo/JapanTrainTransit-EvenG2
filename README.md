...
本アプリケーションは、[Transit](https://transit.ls8h.com)（`https://api.transit.ls8h.com`）の公開APIを利用して経路検索を行っています。

Transit は日本の公共交通情報（GTFS、ODPT、自治体オープンデータ等）をもとにした非公式・無償のサービスです。以下の点を確認した上で利用しています。

- **非公式性の表示**: 本アプリおよびこのREADMEに、Transit が非公式サービスであり正確性・完全性・最新性を保証しない旨を明示しています。
- **公式情報の参照案内**: 実際の乗車や重要な判断には公式情報を確認するよう案内しています（規約「公開APIの利用条件」に対応）。
- **過度なリクエストの回避**: 経路検索は利用者の操作ごとに1回のみ実行し、バッチ取得や高頻度ポーリングは行っていません。
- **公式サービスとの誤認防止**: 本アプリはTransitおよび各交通事業者の公式サービスではありません。

Transit の利用規約・免責事項: https://transit.ls8h.com/terms
